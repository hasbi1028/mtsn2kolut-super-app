<script lang="ts">
  ...
  interface Employee {
    ...
    status_kepegawaian?: string; unit_kerja?: string;
  }
...
<svelte:head><title>Daftar Pegawai — MTsN 2 Kolut</title></svelte:head>

<div class="container-fluid px-0">
  <div class="d-flex align-items-center justify-content-between mb-1">
    <h4 class="fw-black mb-0">Daftar Pegawai</h4>
    <button class="btn btn-outline-success btn-sm" onclick={loadEmployees}>
      <i class="bi bi-arrow-repeat me-1"></i>Refresh
    </button>
  </div>
  <p class="text-secondary mb-3" style="font-size:0.85rem;">Seluruh data pegawai madrasah</p>
              ...
              <tbody>
                {#each filtered as emp, i}
                  <tr>
                    ...
                    <td>{emp.jabatan || '—'}</td>
                    <td>
                      {#if emp.status_kepegawaian}
                        <span class="badge {emp.status_kepegawaian === 'PNS' ? 'bg-primary' : 'bg-success'}">{emp.status_kepegawaian}</span>
                      {:else}—{/if}
                    </td>
                    <td class="text-muted">{emp.unit_kerja || '—'}</td>
                  </tr>
                {/each}
              </tbody>
            </table>
