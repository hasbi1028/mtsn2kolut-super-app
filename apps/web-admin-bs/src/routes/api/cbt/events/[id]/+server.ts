...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const data = await proxy(event).get(apiPath`/api/cbt/events/${id}`);
		return json(data);
	} catch (e) {
...
export const PUT = async (event: RequestEvent) => {
	try {
		...
		const data = await proxy(event).put(apiPath`/api/cbt/events/${id}`, body);
		return json(data);
	} catch (e) {
...
export const DELETE = async (event: RequestEvent) => {
	try {
		const id = requiredRouteParam(event.params.id, 'id');
		await proxy(event).del(apiPath`/api/cbt/events/${id}`);
