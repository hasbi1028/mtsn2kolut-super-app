...
export const POST: RequestHandler = async (event) => {
	try {
		...
		const data = await proxy(event).post(apiPath`/api/system/maintenance/windows/${id}/activate`, body);
		return Response.json({ data });
	} catch (error) {
		return handleRouteError(error, 'system/maintenance/windows/[id]/activate POST');
	}
};
