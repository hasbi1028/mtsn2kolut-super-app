...
export function requireAuthHeaders(accessToken?: string): Record<string, string> {
  return { 'Content-Type': 'application/json', ...requireAuthorizationHeader(accessToken) };
}
...
async function parseEnvelope<T>(res: Response): Promise<ApiEnvelope<T> | undefined> {
  ...
  try { return JSON.parse(text) as ApiEnvelope<T>; } catch { return undefined; }
}
...
async function unwrap<T>(res: Response): Promise<T> {
  ...
  if (upstreamError) throw new ApiError(502, GENERIC_UPSTREAM_ERROR, upstreamError);
  if (!('data' in payload)) {
    throw new ApiError(502, GENERIC_UPSTREAM_ERROR, INVALID_UPSTREAM_JSON);
  }
...
export async function jsonProxyResponse<T, R = T>(
  res: Response,
  options: { fallbackMessage?: string; status?: number; map?: (payload: T) => R } = {}
): Promise<Response> {
  const payload = await readProxyJson<T>(res, options.fallbackMessage);
  const status = options.status ?? res.status;
  if (status === 204) return new Response(null, { status: 204 });
  return jsonResponse(options.map ? options.map(payload) : payload, { status });
}

export async function streamProxyResponse(res: Response, options: {
  fallbackMessage?: string; defaultContentType?: string; defaultCacheControl?: string; headers?: readonly string[];
} = {}): Promise<Response> {
  if (!res.ok) {
...
export function requiredRouteParam(value: string | undefined, name: string): string {
  if (!value) throw new ApiError(400, `${name} tidak valid`);
  return value;
}

export async function readRequestJson<T>(request: Request, maxBytes = DEFAULT_JSON_BODY_LIMIT_BYTES): Promise<T> {
  try { return JSON.parse(await readLimitedRequestText(request, maxBytes)) as T; }
  catch (e) { if (isMalformedJsonError(e)) throw new RequestPayloadError(); throw e; }
}
...
export async function readOptionalRequestJson<T>(request: Request, fallback: T, maxBytes = DEFAULT_JSON_BODY_LIMIT_BYTES): Promise<T> {
  ...
  try { return JSON.parse(text) as T; } catch { throw new RequestPayloadError(); }
}

export async function readLimitedRequestText(request: Request, maxBytes: number): Promise<string> {
  const contentLength = request.headers.get('content-length');
  if (contentLength && Number(contentLength) > maxBytes) throw new RequestPayloadError('Payload JSON terlalu besar');
  ...
  while (true) {
    ...
    if (size > maxBytes) throw new RequestPayloadError('Payload JSON terlalu besar');
...
export function createApiClient(event: RequestEvent) {
  ...
  return {
    ...
    del: <T>(path: string, body?: unknown) => apiRequest<T>(event.fetch, path, {
      method: 'DELETE', body: jsonBody(body),
    }, { ...requireAuthHeaders(currentAccessToken()), ...requestIdHeader }),
    fetch: (path: string, init?: RequestInit) => {
      ...
      return event.fetch(`${getBase()}${path}`, { ...init, headers: { ...initHeaders, ...headers } });
    },
  };
}
...
export async function apiPublicGetWithFetch<T>(fetcher: Fetcher, path: string): Promise<T> {
  return apiRequest<T>(fetcher, path, { method: 'GET' }, publicHeaders());
}

export async function apiPublicPostWithFetch<T>(fetcher: Fetcher, path: string, body?: unknown): Promise<T> {
  return apiRequest<T>(fetcher, path, { method: 'POST', body: jsonBody(body) }, publicHeaders());
}

export async function apiLoginWithFetch(
  fetcher: Fetcher, username: string, password: string, meta?: ClientMeta
): Promise<TokenPair> {
...
export async function apiRefreshWithFetch(
  fetcher: Fetcher, refresh_token: string, meta?: ClientMeta
): Promise<TokenPair> {
...
export async function apiValidateAuthWithFetch(fetcher: Fetcher, accessToken: string): Promise<boolean> {
  let res: Response;
  try {
    res = await fetcher(`${getBase()}${AUTH_VALIDATION_PROBE_PATH}`, {
...
export async function getVerifiedUserFromAccessToken(
  fetcher: Fetcher, accessToken: string, decodeUser: (token: string) => AuthUser | null
...
export function handleRouteError(e: unknown, route = ''): Response {
  ...
  return jsonResponse({ error: 'Backend tidak dapat dihubungi — pastikan Go API berjalan' }, { status: 503 });
}
