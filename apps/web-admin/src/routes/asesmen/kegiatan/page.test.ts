...
describe('/asesmen/kegiatan page', () => {
	it('persists existing event status changes through the status endpoint', async () => {
		...
		const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
			...
			if (url === '/api/asesmen/events' && method === 'GET') {
			...
			if (url === '/api/asesmen/events/event-1' && method === 'PUT') {
				return jsonResponse({ data: { status: 'updated' } });
			}
			if (url === '/api/asesmen/events/event-1/status' && method === 'PATCH') {
		...
		await waitFor(() => {
			const statusCall = fetchMock.mock.calls.find(([url, init]) =>
				String(url) === '/api/asesmen/events/event-1/status' && init?.method === 'PATCH'
			);
			expect(statusCall).toBeTruthy();
		});

		const putCall = fetchMock.mock.calls.find(([url, init]) =>
			String(url) === '/api/asesmen/events/event-1' && init?.method === 'PUT'
		);
		const statusCall = fetchMock.mock.calls.find(([url, init]) =>
			String(url) === '/api/asesmen/events/event-1/status' && init?.method === 'PATCH'
		);
