<script lang="ts">
	...
	function fetchAssessmentsPath() {
		...
		return clientApiPathWithQuery('/api/asesmen/non-test-assessments', params);
	}
	...
	async function saveAssessment() {
		...
		try {
			const path = editingId
				? clientApiPath`/api/asesmen/non-test-assessments/${editingId}`
				: '/api/asesmen/non-test-assessments';
	...
	async function deleteAssessment(item: NonTestAssessment) {
		...
		try {
			const response = await fetch(clientApiPath`/api/asesmen/non-test-assessments/${item.id}`, { method: 'DELETE' });
			await readClientJson<unknown>(response);
			operationState = {
				tone: 'warning',
	...
	async function fetchSubmissions(item: NonTestAssessment): Promise<NonTestSubmission[]> {
		const payload = await fetch(clientApiPath`/api/asesmen/non-test-assessments/${item.id}/submissions`)
	...
	async function generateSubmissions(item: NonTestAssessment) {
		...
		try {
			const response = await fetch(clientApiPath`/api/asesmen/non-test-assessments/${item.id}/submissions/generate`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
	...
	async function syncToGrade(item: NonTestAssessment) {
		...
		try {
			const response = await fetch(clientApiPath`/api/asesmen/non-test-assessments/${item.id}/sync-grade`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
	...
	async function saveSubmission(row: NonTestSubmission) {
		...
		try {
			const response = await fetch(clientApiPath`/api/asesmen/non-test-assessments/${assessment.id}/submissions`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
