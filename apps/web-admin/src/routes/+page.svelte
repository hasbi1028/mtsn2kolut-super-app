<script>
  ...
  async function load() {
    try {
      ...
      if (q.error)  console.error('[mtsn2kolut] queue/stats:', q.error);
      else          queueStats = q;
      if (j.error)  console.error('[mtsn2kolut] jobs:', j.error);
      else          recentJobs = j.items ?? [];
    } catch (e) {
      console.error('[mtsn2kolut] load failed:', e);
    }
  }
  ...
  async function act(key, fn, successMsg) {
    ...
    try {
      ...
      console.error('[mtsn2kolut] action error:', e.message);
...
<header class="card hero">
  <div>
    <h1>MTSN 2 Kolut Super App</h1>
    <p>Dashboard operasional modul absensi dan worker integrasi Pusaka</p>
  </div>
  <div class="btn-group">
