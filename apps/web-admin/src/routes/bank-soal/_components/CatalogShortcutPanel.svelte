...
<section class="rounded-xl border border-border bg-card p-4 shadow-sm">
	<div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
		<div class="min-w-0">
			...
			<p class="mt-1 text-xs text-muted-foreground">Aksi penting untuk mengisi, meninjau, dan memakai repositori soal reusable.</p>
		</div>
		<div class="flex flex-wrap gap-2">
			<Button onclick={onCreate} class="bg-primary text-primary-foreground hover:bg-primary/90">Buat Soal</Button>
			<Button variant="outline" onclick={onImport} class="bg-card">Upload CSV</Button>
