...
<div class="flex min-h-[70vh] items-center justify-center">
	<div class="w-full max-w-2xl rounded-[2rem] border border-base-300 bg-base-100 px-6 py-10 text-center shadow-sm sm:px-10">
		<p class="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Kode {status}</p>
		<h1 class="mt-4 text-4xl font-bold text-base-content sm:text-5xl">{title}</h1>
		<p class="mx-auto mt-4 max-w-xl text-base leading-8 text-base-content/70">{description}</p>
		{#if status >= 500}
		...
		<div class="mt-8 flex flex-wrap justify-center gap-3">
			<a href={resolve('/')} class="rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-content shadow-sm hover:brightness-105">
				Kembali ke Beranda
			</a>
			{#if status >= 500}
			...
			{/if}
			<a href={resolve('/login')} class="rounded-full border border-base-300 px-5 py-3 text-sm font-semibold text-base-content hover:bg-base-200/50">
				Masuk
			</a>
		</div>
	</div>
