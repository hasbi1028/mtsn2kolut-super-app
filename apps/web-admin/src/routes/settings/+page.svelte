<script lang="ts">
  ...
  async function load() {
    try {
      ...
      if (st.error) console.error('[mtsn2kolut] settings:', st.error);
      else          appSettings = st;
      if (s.error)  console.error('[mtsn2kolut] schedules:', s.error);
      else          schedules = s.items ?? [];
    } catch (e) {
      console.error('[mtsn2kolut] settings load failed:', e);
    }
  }
  ...
  async function saveSettings() {
    try {
      ...
      if (!res.ok) { console.error('[mtsn2kolut] save settings error:', data.error); showToast('Gagal menyimpan: ' + (data.error ?? res.status)); return; }
      showToast('Pengaturan worker disimpan.');
    } catch (e) {
      console.error('[mtsn2kolut] save settings failed:', e);
  ...
  async function saveSchedules() {
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { console.error('[mtsn2kolut] save schedules error:', data.error); showToast('Gagal menyimpan jadwal'); return; }
      showToast('Jadwal otomatis disimpan.');
      await load();
    } catch (e) {
      console.error('[mtsn2kolut] save schedules failed:', e);
...
<svelte:head><title>Pengaturan — MTSN 2 Kolut Super App</title></svelte:head>
