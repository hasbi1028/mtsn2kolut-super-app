...
export const GET = async (event: RequestEvent) => {
	try {
		const result = await proxy(event).get<any>(`/api/class-journal/sessions/${event.params.id}/attendances`);
...
export const PUT = async (event: RequestEvent) => {
	try {
		...
		const result = await proxy(event).put<any>(`/api/class-journal/sessions/${event.params.id}/attendances`, body);
		return new Response(JSON.stringify(result), { status: 200, headers: { 'content-type': 'application/json' } });
	} catch (e) {
