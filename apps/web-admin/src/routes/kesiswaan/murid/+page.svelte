...
<div class="space-y-4 max-w-screen-xl mx-auto">
  ...
  {#if filtered().length === 0}
    <Card.Root><Card.Content class="p-8 text-center"><p class="text-sm text-muted-foreground">{#if murid.length === 0}Belum ada data murid.{:else}Tidak ditemukan.{/if}</p></Card.Content></Card.Root>
  {:else}
    <!-- Desktop: Table -->
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-border">
          {#each filtered() as m (m.id)}<tr class="hover:bg-muted/10">
            ...
            <td class="px-3 py-2 text-right whitespace-nowrap"><button class="text-xs text-primary hover:underline" onclick={() => openEdit(m)}>Edit</button></td>
          </tr>{/each}
        </tbody>
      </table>
    </div>
    ...
    <div class="md:hidden space-y-2">
      {#each filtered() as m (m.id)}
        <div class="rounded-xl border border-border bg-base-100 shadow-sm p-3 space-y-1">
          ...
          <div class="pt-1"><button class="text-xs text-primary hover:underline" onclick={() => openEdit(m)}>Edit Profil</button></div>
        </div>
      {/each}
    </div>
  {/if}
