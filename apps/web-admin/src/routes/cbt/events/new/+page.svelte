<script lang="ts">
	...
	async function createEvent() {
		...
		fBusy = true;
		try {
			const res = await fetch('/api/cbt/events', {
				method: 'POST',
	...
	<section class="rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-lime-50 p-6 shadow-sm">
		<div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
			...
			<div class="flex flex-wrap gap-2">
				<Button href={resolve('/cbt/persiapan')} variant="outline">Persiapan CBT</Button>
				<Button href={resolve('/cbt/events')} variant="outline">Batal</Button>
			</div>
		</div>
	</section>
		...
		{#snippet children()}
			<Card.Root class="border-slate-200 shadow-sm">
				<Card.Header>
					...
					<Card.Description>Payload mengikuti endpoint <code>/api/cbt/events</code> yang dipakai form lama.</Card.Description>
				</Card.Header>
