<script lang="ts">
	...
	async function fetchPayload(): Promise<PagePayload> {
		const [nextInfo, academic, nextUsers, nextMembers] = await Promise.all([
			fetch(clientApiPath`/api/cbt/events/${eventId}`).then((response) => readClientApiData<CbtEvent>(response, 'Gagal memuat kegiatan CBT')),
			fetch('/api/cbt/soal-support/subjects').then((response) => readClientApiData<AcademicPayload>(response, 'Gagal memuat mapel')),
			fetch('/api/users').then((response) => readClientApiData<UsersPayload>(response, 'Gagal memuat pengguna')),
			fetch(clientApiPath`/api/cbt/events/${eventId}/members`).then((response) => readClientApiData<EventMember[]>(response, 'Gagal memuat penugasan'))
		]);
	...
	async function deleteMember(member: EventMember) {
		...
		try {
			await fetch(clientApiPath`/api/cbt/events/${eventId}/members/${member.id}`, { method: 'DELETE' }).then((response) => readClientJson<unknown>(response));
...
<div class="space-y-6 p-6">
	<div class="flex items-center gap-2 text-sm text-slate-500">
		...
		<span>/</span>
		<a href={resolve(`/cbt/events/${eventId}`)} class="hover:text-slate-700">{info?.title ?? 'Detail'}</a>
		<span>/</span>
	...
	<AsyncContent promise={payloadPromise}>
		...
		{#snippet children(value)}
			...
			<div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
				...
				<a href={resolve(`/cbt/soal?event_id=${eventId}`)} class="inline-flex rounded-md border border-green-200 bg-green-50 px-3 py-2 text-sm font-semibold text-green-800 hover:bg-green-100">Buka Bank Soal Kegiatan</a>
			</div>
