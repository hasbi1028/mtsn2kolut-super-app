<script lang="ts">
	...
	async function fetchRooms() {
		const response = await fetch('/api/cbt/proctoring/my-rooms');
					...
					{#each filteredRooms as room (room.id)}
						<article class="border border-emerald-100 bg-white p-4 shadow-sm">
							...
							<div class="mt-4 flex flex-wrap gap-2">
								<Button href={resolve(`/cbt/sessions/${room.session_id}/rooms/${room.id}/proctoring`)}>
									<ActivityIcon class="mr-2 size-4" />
									Dashboard
								</Button>
								<Button href={resolve(`/cbt/sessions/${room.session_id}/rooms/${room.id}/print-pack`)} variant="outline">
									<PrinterIcon class="mr-2 size-4" />
									Paket Cetak
								</Button>
								<Button href={resolve(`/cbt/sessions/${room.session_id}`)} variant="outline">
									...
									<Table.Row>
										...
										<Table.Cell class="text-right">
											<div class="flex flex-wrap justify-end gap-2">
												<Button size="sm" href={resolve(`/cbt/sessions/${room.session_id}/rooms/${room.id}/proctoring`)} variant="outline">Dashboard</Button>
												<Button size="sm" href={resolve(`/cbt/sessions/${room.session_id}/rooms/${room.id}/print-pack`)} variant="outline">Cetak</Button>
											</div>
										</Table.Cell>
									</Table.Row>
								{/each}
