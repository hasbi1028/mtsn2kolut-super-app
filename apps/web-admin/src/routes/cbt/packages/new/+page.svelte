<script lang="ts">
	...
	let listHref = $derived(`${resolve('/cbt/packages')}${eventId ? `?event_id=${encodeURIComponent(eventId)}` : ''}`);
	...
	async function createPackage() {
		...
		try {
			...
			await goto(eventId ? `${resolve('/cbt/packages')}?event_id=${encodeURIComponent(eventId)}` : resolve('/cbt/packages'));
		} catch (error) {
		...
		{#snippet children()}
			<Card.Root class="border-emerald-200 shadow-sm">
				<Card.Header>
					...
					<Card.Description>Form create-only ini memakai endpoint <code>/api/cbt/packages</code> dan filter soal yang sama dengan builder lama.</Card.Description>
				</Card.Header>
