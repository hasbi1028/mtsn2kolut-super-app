<script lang="ts">
	...
	type PelaksanaanRoute = '/cbt/byod' | '/cbt/sessions' | '/cbt/proctoring/rooms' | '/cbt/events' | '/cbt/persiapan' | '/cbt';
...
{#if canAccess}
	...
	<section class="rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-lime-50 p-6 shadow-sm">
		<div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
			...
			<div class="flex flex-wrap gap-3">
				{#if canOpenPersiapan}
					<Button href={resolve('/cbt/persiapan')} variant="outline">Kembali ke Persiapan</Button>
				{:else}
					<Button href={resolve('/cbt')} variant="outline">Kembali ke Dashboard</Button>
				{/if}
				<Button href={resolve('/cbt/byod')}>Panduan BYOD</Button>
			</div>
		</div>
	</section>
	...
	<section aria-labelledby="pelaksanaan-focus-title" class="grid gap-4 lg:grid-cols-[1fr_18rem]">
		...
		<Card.Root class="h-fit border-emerald-200 bg-emerald-50/60 shadow-sm">
			...
			<Card.Footer>
				<Button href={resolve('/cbt')} variant="outline" class="w-full border-emerald-200 text-emerald-800 hover:bg-emerald-50">Beranda CBT</Button>
			</Card.Footer>
		</Card.Root>
	</section>
</div>
