<script lang="ts">
	...
	let listHref = $derived(`${resolve('/cbt/sessions')}${eventId ? `?event_id=${encodeURIComponent(eventId)}` : ''}`);
	...
	async function createSession() {
		...
		try {
			...
			await goto(eventId ? `${resolve('/cbt/sessions')}?event_id=${encodeURIComponent(eventId)}` : resolve('/cbt/sessions'));
		} catch (error) {
	...
	<section class="rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-lime-50 p-6 shadow-sm">
		<div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
			...
			<div class="flex flex-wrap gap-2">
				{#if eventId}
					<Button href={resolve(`/cbt/events/${eventId}`)} variant="outline">Kembali ke Event</Button>
				{/if}
	...
	{#if eventId}
		<div class="rounded-xl border border-green-200 bg-green-50/70 p-4 text-sm text-green-950">
			<div class="flex flex-wrap items-start justify-between gap-3">
				...
				<Button href={resolve(`/cbt/packages/new?event_id=${eventId}`)} variant="outline" size="sm">Buat Paket Event</Button>
			</div>
		</div>
	{:else}
		...
		{#snippet children()}
			<Card.Root class="border-slate-200 shadow-sm">
				<Card.Header>
					...
					<Card.Description>Form create-only ini memakai endpoint <code>/api/cbt/sessions</code> dan quality gate paket dari daftar sesi lama.</Card.Description>
				</Card.Header>
