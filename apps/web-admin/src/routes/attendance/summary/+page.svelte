<script lang="ts">
	...
	async function load() {
		...
		try {
			...
			error = 'Gagal memuat ringkasan absensi';
		} finally {
			loading = false;
		}
	}
	...
	function exportCSV() {
		...
		a.href = url;
		a.download = `summary_absensi_${startDate}_${endDate}.csv`;
		a.click();
...
<svelte:head><title>Ringkasan Absensi Bulanan — MTSN 2 Kolut</title></svelte:head>

<div class="space-y-6 p-6">
	<div class="flex items-center gap-2 text-sm text-slate-500">
		<a href="/attendance" class="hover:text-slate-700">Absensi</a>
		<span>/</span>
		<span class="text-slate-700 font-medium">Ringkasan Bulanan</span>
	</div>

	<div class="flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-semibold text-slate-800">Ringkasan Absensi</h1>
			<p class="text-sm text-slate-500 mt-1">Akumulasi kehadiran pegawai dalam periode tertentu</p>
		</div>
	...
	{#if error}<div class="rounded-md bg-red-50 border border-red-200 p-4 text-sm text-red-800">{error}</div>{/if}
							...
							<Table.Cell class="text-center">
								<div class="flex items-center justify-center gap-2">
									<div class="w-12 h-1.5 rounded-full bg-slate-100 overflow-hidden">
										<div class="h-full bg-green-600" style="width: {percent}%"></div>
									</div>
									<span class="text-xs font-mono w-8">{Math.round(percent)}%</span>
								</div>
							</Table.Cell>
						</Table.Row>
					{:else}
						<Table.Row>
							<Table.Cell colspan={7} class="py-12 text-center text-slate-400">
								{loading ? 'Memuat...' : 'Tidak ada data summary.'}
							</Table.Cell>
						</Table.Row>
					{/each}
				</Table.Body>
