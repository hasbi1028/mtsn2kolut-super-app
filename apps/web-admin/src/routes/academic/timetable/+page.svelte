<script lang="ts">
  ...
  async function submitSlot() {
    ...
    slotLoading = true;
    try {
      const url = editMode && editSlotId
        ? `/api/academic/timetable/slots/${editSlotId}`
        : '/api/academic/timetable/slots';
      const method = editMode && editSlotId ? 'PUT' : 'POST';
      ...
      slotError = 'Terjadi kesalahan, coba lagi';
    }
  ...
  type Conflict = {
    ...
    day: number; time_range: string;
    slots: Slot[]; description: string;
  };
  ...
  let conflicts = $derived.by(() => {
    ...
    for (const s of slots) {
      ...
      if (s.teacher_name) { const k = groupKey(s.day_of_week, s.teacher_name); if (!byTeacher[k]) byTeacher[k] = []; byTeacher[k].push(s); }
      if (s.class_code) { const k = groupKey(s.day_of_week, s.class_code); if (!byClass[k]) byClass[k] = []; byClass[k].push(s); }
      if (s.room_label) { const k = groupKey(s.day_of_week, s.room_label); if (!byRoom[k]) byRoom[k] = []; byRoom[k].push(s); }
    }
      ...
      for (let i = 0; i < items.length; i++) {
        for (let j = i + 1; j < items.length; j++) {
          if (timeOverlap(items[i], items[j])) {
            ...
            result.push({ type: 'teacher_overlap', teacher_name: items[i].teacher_name, day, time_range: `${timeLabel(items[i])} / ${timeLabel(items[j])}`, slots: [items[i], items[j]], description: `👨‍🏫 ${items[i].teacher_name} mengajar "${items[i].subject_name}" (${items[i].class_code}) dan "${items[j].subject_name}" (${items[j].class_code}) di jam bersamaan!` });
          }
        }
      }
    }
    for (const [key, items] of Object.entries(byClass)) {
      for (let i = 0; i < items.length; i++) {
        for (let j = i + 1; j < items.length; j++) {
          if (timeOverlap(items[i], items[j])) {
            const day = Number(key.split('|')[0]);
            result.push({ type: 'class_overlap', class_name: items[i].class_code, day, time_range: `${timeLabel(items[i])} / ${timeLabel(items[j])}`, slots: [items[i], items[j]], description: `🏫 ${items[i].class_code} diampu "${items[i].subject_name}" (${items[i].teacher_name}) dan "${items[j].subject_name}" (${items[j].teacher_name}) di jam bersamaan!` });
          }
        }
      }
    }
    for (const [key, items] of Object.entries(byRoom)) {
      for (let i = 0; i < items.length; i++) {
        for (let j = i + 1; j < items.length; j++) {
          if (timeOverlap(items[i], items[j]) && items[i].class_code !== items[j].class_code) {
            const day = Number(key.split('|')[0]);
            result.push({ type: 'room_overlap', class_name: items[i].class_code, day, time_range: `${timeLabel(items[i])} / ${timeLabel(items[j])}`, slots: [items[i], items[j]], description: `🚪 Ruang ${items[i].room_label} dipakai ${items[i].class_code} dan ${items[j].class_code} di jam bersamaan!` });
          }
        }
      }
    }
...
<div class="space-y-4 max-w-screen-xl mx-auto pb-8">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
    <div>
      ...
      <p class="text-sm text-muted-foreground">Atur jadwal mingguan per rombel. Tambah, edit, atau hapus slot jadwal.</p>
    </div>
  </div>
    ...
    <div class="rounded-xl border-2 border-destructive/20 bg-destructive/5 p-4 space-y-2">
      ...
      <div class="space-y-1">
        {#each conflicts as c, idx (idx)}
          <div class="flex items-start gap-2 text-xs px-1"><span class="shrink-0 mt-0.5">{c.type === 'teacher_overlap' ? '👨‍🏫' : c.type === 'class_overlap' ? '🏫' : '🚪'}</span><div><span class="text-foreground font-medium">{dayName(c.day)}</span><span class="text-muted-foreground"> — {c.description}</span></div></div>
        {/each}
      </div>
    </div>
  {/if}
  ...
  {#if classes.length === 0}
    <Card.Root><Card.Content class="p-10 text-center space-y-2">
      <p class="text-2xl">📭</p>
      <p class="text-sm text-muted-foreground">Belum ada data jadwal. Pastikan assign guru sudah dilakukan di menu Akademik → Assign Guru.</p>
                          ...
                          {#if slot}
                            <div class="rounded-md border px-1.5 py-1.5 {conflictSlotIds.has(slot.id) ? 'border-destructive/30 bg-destructive/10' : 'border-primary/10 bg-primary/5'}">
                              ...
                              <div class="flex gap-1.5 justify-end mt-1">
                                ...
                                <button class="text-[8px] text-destructive hover:underline font-semibold" onclick={() => confirmDelete(slot.id)}>Hapus</button>
                              </div>
                            </div>
                          {:else}
              ...
              {#each days as d, idx}
                ...
                <div>
                  <h4 class="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5 flex items-center gap-2">
                    {d}
                    {#if daySlots.some(s => conflictSlotIds.has(s.id))}<span class="text-destructive text-[9px]">⚠️ bentrok</span>{/if}
                  </h4>
                      ...
                      {#each daySlots as s (s.id)}
                        <div class="flex items-center justify-between gap-2 rounded-lg border px-2.5 py-2.5 {conflictSlotIds.has(s.id) ? 'border-destructive/30 bg-destructive/5' : 'border-border'}">
                          ...
                          <div class="flex gap-1.5 shrink-0">
                            ...
                            <button class="text-[10px] text-destructive hover:underline font-semibold" onclick={() => confirmDelete(s.id)}>Hapus</button>
                          </div>
                        </div>
                      {/each}
                    </div>
        ...
        <tbody>
          {#each conflicts as c, idx (idx)}
            <tr class="{idx % 2 === 0 ? '' : 'bg-muted/20'}">
              ...
              <td class="py-1.5 pr-2">{#if c.type === 'teacher_overlap'}<Badge variant="destructive">Guru</Badge>{:else if c.type === 'class_overlap'}<Badge variant="destructive">Kelas</Badge>{:else}<Badge variant="destructive">Ruang</Badge>{/if}</td>
  ...
  <Dialog.Content>
    <div class="space-y-4">
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        ...
        <div class="space-y-1">
          <label class="text-xs font-medium text-muted-foreground">Hari</label>
        ...
        <div class="space-y-1">
          <label class="text-xs font-medium text-muted-foreground">Jam Mulai</label>
          <Input type="time" bind:value={slotForm.start_time} />
        </div>
        <div class="space-y-1">
          <label class="text-xs font-medium text-muted-foreground">Jam Selesai</label>
          <Input type="time" bind:value={slotForm.end_time} />
        </div>
        <div class="space-y-1">
          <label class="text-xs font-medium text-muted-foreground">Ruang</label>
      ...
      <div class="flex justify-end gap-2 pt-2">
        ...
        <LoadingButton onclick={() => void submitSlot()} loading={slotLoading}>
          {editMode ? 'Simpan Perubahan' : 'Tambah Slot'}
        </LoadingButton>
      </div>
    </div>
  </Dialog.Content>
...
<Dialog.Root bind:open={deleteDialogOpen}>
  <Dialog.Content class="max-w-sm">
    <div class="space-y-4 text-center">
      ...
      <p class="text-xs text-muted-foreground">Slot yang dihapus tidak bisa dikembalikan. Lanjutkan?</p>
