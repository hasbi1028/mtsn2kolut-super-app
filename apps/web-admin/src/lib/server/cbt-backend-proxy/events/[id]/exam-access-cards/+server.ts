...
function cardPath(event: RequestEvent, suffix = '') {
	...
	return cbtBackendPathWithQuery(`/events/${id}/exam-access-cards${suffix}`, event.url.searchParams);
}
...
export const GET = async (event: RequestEvent) => {
	try {
		...
		return handleRouteError(e, 'cbt/events/[id]/exam-access-cards GET');
	}
};
...
export const POST = async (event: RequestEvent) => {
	try {
		...
		return handleRouteError(e, 'cbt/events/[id]/exam-access-cards/issue POST');
	}
};
