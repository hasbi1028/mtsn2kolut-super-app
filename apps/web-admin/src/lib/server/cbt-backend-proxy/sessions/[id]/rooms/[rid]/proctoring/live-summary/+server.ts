...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const data = await proxy(event).get(cbtApiPath`/sessions/${id}/rooms/${rid}/proctoring/live-summary`);
		return json(data);
	} catch (e) {
		return handleRouteError(e, 'cbt/sessions/[id]/rooms/[rid]/proctoring/live-summary GET');
	}
};
