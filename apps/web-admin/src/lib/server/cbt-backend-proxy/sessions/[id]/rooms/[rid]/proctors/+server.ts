...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const data = await proxy(event).get(cbtApiPath`/sessions/${id}/rooms/${rid}/proctors`);
		return json(data);
	} catch (e) {
...
export const PUT = async (event: RequestEvent) => {
	try {
		...
		const data = await proxy(event).put(cbtApiPath`/sessions/${id}/rooms/${rid}/proctors`, body);
		return json(data);
	} catch (e) {
