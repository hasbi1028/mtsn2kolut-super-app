...
export const GET = async (event: RequestEvent) => {
	try {
		const data = await proxy(event).get(queryPath(cbtBackendPath('/sessions'), event.url.searchParams));
		return json(data);
	} catch (e) {
...
export const POST = async (event: RequestEvent) => {
	try {
		...
		const data = await proxy(event).post(cbtBackendPath('/sessions'), { ...body, status: status ?? 'draft' });
