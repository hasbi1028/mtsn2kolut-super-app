<script lang="ts">
  ...
  let { form, onclose }: {
    form?: { tambahError?: string; tambahSuccess?: string };
    onclose?: () => void;
  } = $props();
...
<form method="POST" action="?/tambah" use:enhance>
  ...
  <div class="mt-5 flex items-center justify-end gap-2">
    {#if onclose}
      <button type="button" onclick={onclose} class="inline-flex h-10 min-w-28 items-center justify-center gap-2 rounded-lg border border-input bg-background px-4 text-sm font-medium text-foreground hover:bg-accent transition-colors">
        Batal
      </button>
    {/if}
