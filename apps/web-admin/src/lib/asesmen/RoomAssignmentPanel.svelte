<script lang="ts">
	...
	const assignmentPayload = $derived(buildRoomAssignmentPayload(mixPolicy));
	...
	async function previewRooms() {
		...
		try {
			...
			preview = await readJson<RoomAssignmentPreview>(response);
		...
		<div class="mt-4 grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
			<div class="space-y-4">
				<label class="block space-y-1 text-sm font-medium text-foreground" for="room-assignment-session">
					...
					<select id="room-assignment-session" class="min-h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" bind:value={selectedSessionId} onchange={resetPreview}>
				...
				<div class="space-y-2">
					...
					{#each roomAssignmentOptions as option}
						<label class={`flex gap-3 rounded-lg border p-3 text-sm ${mixPolicy === option.value ? 'border-primary/40 bg-primary/10' : 'border-border bg-background'}`}>
							<input type="radio" bind:group={mixPolicy} value={option.value} onchange={resetPreview} />
				...
				<div class="grid grid-cols-2 gap-2">
					...
					<button class="min-h-10 rounded-md bg-foreground px-3 text-sm font-semibold text-background disabled:opacity-50" disabled={roomWorking || !canSavePreview} onclick={applyRooms}>Simpan</button>
				</div>
			</div>
