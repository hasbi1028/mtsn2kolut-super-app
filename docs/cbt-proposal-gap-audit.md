...
| Question types | 4.2 Tipe Soal yang Didukung | Partial | Pilihan ganda, essay/uraian, rich stem/stimulus, assets, KaTeX-ready authoring, import/export, and manual essay grading are present. Multi-select, isian singkat, matching, true/false, ordering, hotspot, upload-answer, and full audio/video scoring are not all complete. | Treat additional item types as backlog with schema/API tests, Flutter rendering tests, and scoring semantics before release. |
