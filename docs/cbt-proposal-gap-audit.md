...
| Question types | 4.2 Tipe Soal yang Didukung | Partial | Pilihan ganda, essay/uraian, rich stem/stimulus, assets, KaTeX-ready authoring, import/export, and manual essay grading are present. Flutter runtime now recognizes ordering questions, renders reorder controls from the existing options list, restores saved order, and sends comma-separated labels to `/api/exam/answer`; ordering authoring/scoring may still be partial. Multi-select, isian singkat, matching, true/false, hotspot, upload-answer, and full audio/video scoring are not all complete. | Treat additional item types as backlog with schema/API tests, Flutter rendering tests, and scoring semantics before release. |
