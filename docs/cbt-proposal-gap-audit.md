...
| ISO controls | 5.3, 6.1, 6.2, 6.3 | Partial | RBAC, JWT refresh sessions, auth audit events, rate-limit/trusted-proxy baseline, upload hygiene, generic 500 hygiene, docs guard, manifest/checksum evidence, and secret-scan evidence are present. Phase 29 records Security and ISO-Control Alignment Evidence as control alignment, not certification. Formal ISO certification, complete ISMS/QMS controls, penetration test, and semester disaster-recovery tests require manual/ops evidence. | Record ISO as control alignment, not certification; attach manual security/ops evidence before any external claim. |
| Infrastructure | 5.1 Stack Teknologi, 5.2 Arsitektur Sistem, 9.2 Kebutuhan Infrastruktur | Out of scope adapted | Approved runtime is 3 VPS targets with native PM2, SvelteKit Web Admin, Go Core API, PostgreSQL, PUSAKA worker, and Flutter APK. Phase 28 records Infrastructure, Backup, Restore, and DR Evidence through backup path, latest symlink, checksum, and `pg_restore --list` verification where available. No PocketBase, runtime SQLite, Alpine-only UI, Caddy requirement, Docker runtime, or single-server collapse is introduced. | Preserve service boundaries and deploy order; gather `make ops-health` and backup verification evidence as read-only status, not deploy automation. Do not restore over live DB. |
| Risks | 11 Manajemen Risiko | Planned manual evidence | Some mitigations exist in code/docs: server-side timer, autosave/local pending answers, heartbeat, restore, auth hardening, backups/runbooks, audit evidence templates, and Phase 27-30 checklists. UPS, LAN stability, operator behavior, real-device policy, backup restore, and security audit evidence require physical/ops rehearsal. | Complete device matrix, operator rehearsal, backup/DR evidence review, security control-alignment review, mobile RC build/hash evidence, and final go/no-go sign-off before production acceptance. |
...
## Phase 27-30 follow-up status (2026-05-08)

- Phase 27 Operator Rehearsal Workflow Completion: `docs/cbt-proposal-integration-phase-27-30.md` and `docs/cbt-release-evidence-template.md` require Bank Soal to Asesmen Persiapan to Pelaksanaan/Pengawasan to Flutter APK to Hasil/Post-exam review, proctor evidence, role/scope/token boundary, event/audit evidence, and go/no-go rehearsal. Manual evidence remains pending until operator/pengawas attach the rehearsal record.
