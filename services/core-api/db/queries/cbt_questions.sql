...
WHERE (
  ...
  AND (sqlc.arg(difficulty_filter)::text = '' OR q.difficulty::text = sqlc.arg(difficulty_filter)::text)
...
WHERE author_username = sqlc.arg(author_username)::text
  ...
  AND COALESCE(difficulty::text, '') = COALESCE(sqlc.arg(difficulty)::text, '')
...
-- name: ListCbtQuestionAuthors :many
