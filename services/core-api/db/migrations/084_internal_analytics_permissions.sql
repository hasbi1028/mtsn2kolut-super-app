...
-- Adds explicit RBAC permissions for aggregate analytics reads. No public collector,
-- export endpoint, or raw event access is introduced by this migration.
...
SELECT r.id, p.id
FROM rbac_roles r
JOIN rbac_permissions p ON p.code IN ('analytics.read', 'analytics.security_read')
WHERE r.code = 'admin'
