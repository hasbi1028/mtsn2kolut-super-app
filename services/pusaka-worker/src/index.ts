...
const DEFAULT_MAX_CONCURRENT = Math.max(1, Number(process.env.WORKER_CONCURRENCY ?? 5));
const DEFAULT_HEADLESS       = ['1', 'true'].includes(process.env.HEADLESS ?? 'true');
const POLL_MS        = Number(process.env.POLL_MS        ?? 8000);
const CONFIG_SYNC_MS = Math.max(5000, Number(process.env.CONFIG_SYNC_MS ?? 30000));
...
const consumers = new Map<number, ConsumerState>();
let nextConsumerNumber = 1;
let lastConfigSyncAt = '';
...
async function fetchRuntimeConfig(): Promise<Partial<RuntimeConfig>> {
	...
	const payload = await res.json() as { data?: { max_concurrent?: string; headless?: string } };
	const data = payload.data ?? {};
	const next: Partial<RuntimeConfig> = {};

	if (data.max_concurrent) {
		const parsed = Number(data.max_concurrent);
		if (Number.isFinite(parsed) && parsed >= 1) next.maxConcurrent = Math.floor(parsed);
	}
	if (data.headless) {
...
async function sendHeartbeat(): Promise<void> {
	const res = await fetch(`${BACKEND_URL}/api/worker/heartbeat`, {
		...
		body: JSON.stringify({
			worker_id: WORKER_ID,
			active_consumers: Array.from(consumers.values()).filter((state) => !state.stopRequested).length,
			target_concurrency: runtimeConfig.maxConcurrent,
			headless: runtimeConfig.headless,
			last_sync_at: lastConfigSyncAt,
		}),
	});
...
async function consumerLoop(state: ConsumerState): Promise<void> {
	...
	while (!state.stopRequested) {
		try {
			...
			log('INFO', 'job claimed', { consumerId: state.consumerId, job_id: job.id, run_type: job.run_type, employee_id: job.employee_id });
			await processJob(job, state.consumerId);
		} catch (e) {
			log('ERROR', 'consumer loop error', { consumerId: state.consumerId, error: (e as Error)?.message ?? String(e) });
...
async function syncRuntimeConfig(): Promise<void> {
	...
	try {
		...
		if (typeof next.maxConcurrent === 'number') {
			runtimeConfig.maxConcurrent = next.maxConcurrent;
		}
		if (typeof next.headless === 'boolean') {
			runtimeConfig.headless = next.headless;
		}

		if (prev.maxConcurrent !== runtimeConfig.maxConcurrent || prev.headless !== runtimeConfig.headless) {
			log('INFO', 'runtime config updated', { previous: prev, current: runtimeConfig });
		}
		lastConfigSyncAt = new Date().toISOString();
		reconcileConsumers();
	} catch (e) {
		log('WARN', 'runtime config sync failed', { error: (e as Error)?.message ?? String(e) });
	}
}

function reconcileConsumers(): void {
	if (shuttingDown) return;
	while (consumers.size < runtimeConfig.maxConcurrent) {
		const id = nextConsumerNumber++;
		const state: ConsumerState = {
			id,
			consumerId: `${WORKER_ID}-c${id}`,
			stopRequested: false,
		};
		consumers.set(id, state);
		consumerLoop(state).catch((e) => {
			log('ERROR', 'consumer crashed', { consumerId: state.consumerId, error: (e as Error)?.message ?? String(e) });
		}).finally(() => {
			consumers.delete(id);
			reconcileConsumers();
		});
	...
	if (consumers.size <= runtimeConfig.maxConcurrent) return;

	const active = Array.from(consumers.values()).sort((a, b) => b.id - a.id);
	let activeCount = consumers.size;
	for (const state of active) {
		if (activeCount <= runtimeConfig.maxConcurrent) break;
		if (state.stopRequested) continue;
		state.stopRequested = true;
		activeCount--;
		log('INFO', 'consumer stop requested', {
			consumerId: state.consumerId,
			targetConcurrency: runtimeConfig.maxConcurrent,
		});
	}
}
...
async function heartbeatLoop(): Promise<void> {
	...
	try {
		...
		log('WARN', 'worker heartbeat failed', { error: (e as Error)?.message ?? String(e) });
	}
}
...
async function gracefulShutdown(signal: string): Promise<void> {
	if (shuttingDown) return;
	shuttingDown = true;
	log('INFO', 'shutdown requested', { signal, activeConsumers: consumers.size });
	...
	if (consumers.size > 0) {
		log('WARN', 'shutdown timeout reached', { remainingConsumers: consumers.size });
	}
	...
	log('INFO', 'worker shutdown complete', { remainingConsumers: consumers.size });
	process.exit(0);
}
