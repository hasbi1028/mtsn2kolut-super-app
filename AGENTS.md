...
- **Session-store baseline** — the Flutter exam session store should keep direct unit coverage for base URL persistence, snapshot roundtrips, invalid snapshot fallback, and clear-vs-remembered-base-URL behavior, because BYOD restore reliability depends on those local persistence guarantees.
