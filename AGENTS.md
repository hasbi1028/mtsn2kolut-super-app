...
- **Mobile snapshot storage baseline** — restore metadata may remain lightweight and human-readable, but sensitive mobile exam snapshot fields such as exam token, device fingerprint, answers, and pending answers should not live in the same plaintext preferences payload as general restore metadata.
...
- **Exam-model parser baseline** — Flutter exam model parsers should keep direct unit coverage for rich question fields, media/audio URLs, safe defaults, and `is_submitted`, so backend contract drift is caught on the client side as well as in Go handler tests.
- **Exam-api client baseline** — the Flutter exam API client should keep direct coverage for wrapped `data` envelopes, `X-Exam-Token` transport, outbound login/heartbeat/answer/event/submit payload contracts, missing-data failures, malformed or non-object JSON mapping, transport-failure mapping, and message extraction precedence (`message` → `error` → generic fallback), so mobile contract drift is caught before it reaches UI state.
- **Transport-guidance baseline** — Flutter exam guidance helpers should explicitly map transport-level failures (`statusCode == null`) for login, restore, answer, and submit flows, because BYOD users and pengawas need different instructions for “server unreachable” versus “server rejected the request.”
- **Transport-guidance widget baseline** — the rendered login, restore-failed, and shell guidance panels should also keep widget coverage for transport-level warnings, so “server unreachable” UI does not silently regress after layout or copy refactors.
- **Session-store baseline** — the Flutter exam session store should keep direct unit coverage for base URL persistence, snapshot roundtrips, invalid or malformed snapshot fallback, and clear-vs-remembered-base-URL behavior, because BYOD restore reliability depends on those local persistence guarantees.
- **Session-snapshot baseline** — the Flutter exam session snapshot model should keep direct unit coverage for `toJson/fromJson` roundtrips, dynamic answer/audio coercion, and safe defaults, because restore safety depends on a stable serialized snapshot contract as much as on the storage wrapper.
