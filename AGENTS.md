...
- **STATE PERSISTENCE WAJIB** — Setiap view state (tab, filter, pagination, draft, wizard step) harus dipersist. Gunakan bertingkat: (1) URL Search Params untuk view/filter, (2) SessionStorage store untuk wizard/flow, (3) Draft auto-save untuk form. Lihat `apps/web-admin/AGENTS.md` → `STATE PERSISTENCE PATTERN` untuk detail implementasi. Utility di `apps/web-admin/src/lib/stores/persistent.ts`.
