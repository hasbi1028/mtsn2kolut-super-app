...
- **Supervisor-intervention baseline** — if `Waspada` persists too long without fresh server contact, the mobile shell should escalate to a visible pengawas-intervention panel and emit a one-time server warning event for that stale episode.
