...
- **Exam-api client baseline** — the Flutter exam API client should keep direct coverage for wrapped `data` envelopes, `X-Exam-Token` transport, missing-data failures, and message extraction precedence (`message` → `error` → generic fallback), so mobile contract drift is caught before it reaches UI state.
