...
- [x] Terapkan quality gate ulang saat sesi dijadwalkan atau dimulai agar sesi lama tetap dicek terhadap perubahan mutu paket terbaru.
- [x] Tambahkan ringkasan kesiapan sesi di daftar CBT: paket, peserta, ruang, kapasitas, penempatan kursi, dan pengawas terlihat dalam satu baris operasional.
- [x] Tambahkan `Aksi Berikutnya` pada daftar/detail sesi agar blocker kesiapan langsung berubah menjadi shortcut kerja operator.
...
### ✅ Sprint 79 — CBT Session Status Package Recheck (COMPLETE)
- [x] Tujuan utama: memastikan sesi lama yang sudah dibuat sebelum guard baru tetap dicek ulang saat akan resmi dijadwalkan atau dimulai.
- [x] Backend `UpdateStatus` sekarang mengambil sesi, membaca `package_id`, lalu menjalankan quality gate paket yang sama untuk status `scheduled` dan `active`.
- [x] Jika paket berubah menjadi nonaktif, kosong, tanpa soal terbit, atau masih punya soal belum terbit, perubahan status ditolak dengan konflik terkontrol sebelum status ditulis.
- [x] Daftar `/cbt/sessions` menampilkan badge mutu paket per sesi: jumlah soal, `Paket siap`, metadata kurang, atau masalah blocking seperti paket nonaktif/kosong/belum terbit.
- [x] Tombol `Jadwalkan` dan `Mulai` disabled di frontend ketika paket sesi punya masalah blocking, sementara backend tetap menjadi sumber kebenaran.
- [x] Guardrail: tidak mengubah kontrak Flutter live exam, tidak menambah migrasi, tidak mengubah status workflow paket, dan tetap memakai query sqlc existing.

### ✅ Sprint 80 — CBT Session Readiness Summary (COMPLETE)
- [x] Tujuan utama: menjadikan daftar sesi CBT sebagai papan kesiapan operasional singkat sebelum operator membuka detail.
- [x] Query `ListCbtExamSessions` dan `ListCbtExamSessionsByTeacher` sekarang membawa metrik readiness: total peserta, ruang, kapasitas, peserta ditempatkan, peserta belum ruang, peserta belum nomor meja, ruang tanpa pengawas, dan jumlah pengawas.
- [x] `/cbt/sessions` menampilkan kolom/kartu `Kesiapan` yang menggabungkan status paket, peserta, ruang, kursi, kapasitas, dan pengawas.
- [x] Badge `Siap mulai` hanya muncul jika tidak ada masalah paket maupun operasional; jika ada blocker, layar menampilkan jumlah atensi dan dua penyebab utama.
- [x] Tombol `Mulai` ikut disabled jika masih ada masalah operasional seperti belum ada peserta, belum ada ruang, kapasitas kurang, peserta belum ruang/meja, atau ruang tanpa pengawas.
- [x] Guardrail: tidak mengubah kontrak Flutter live exam, tidak menambah migrasi, tidak menambah endpoint baru, dan tetap memakai Go API + sqlc + BFF.

### ✅ Sprint 81 — CBT Next Operational Action (COMPLETE)
- [x] Tujuan utama: mengubah ringkasan kesiapan menjadi aksi operator yang jelas dan cepat dieksekusi.
- [x] Daftar `/cbt/sessions` sekarang menghitung `Aksi Berikutnya` per sesi: `Rapikan Paket`, `Daftarkan Peserta`, `Atur Ruang`, `Acak Ruang`, `Atur Nomor Meja`, `Tetapkan Pengawas`, atau `Lihat Detail`.
- [x] Shortcut daftar sesi dapat membuka modal daftar peserta, masuk ke detail sesi dengan `?tab=ruangan`, atau mengarah ke paket soal sesuai blocker paling prioritas.
- [x] Detail sesi sekarang mendukung deep-link tab dari query string, sehingga shortcut daftar bisa langsung membuka tab operasional yang relevan.
- [x] Detail sesi menambahkan kartu `Aksi Berikutnya` berbasis readiness ruangan, termasuk tombol cek kesiapan, atur ruang, atur nomor meja otomatis, tetapkan pengawas, atau pantau proctoring.
- [x] Guardrail: tidak mengubah kontrak Flutter live exam, tidak menambah migrasi, tidak menambah endpoint baru, dan tetap memakai alur UI/API yang sudah ada.

### ✅ Sprint 82 — CBT Session Readiness Filters (COMPLETE)
- [x] Tujuan utama: membuat operator cepat menemukan sesi CBT yang butuh tindakan tanpa membuka satu per satu detail sesi.
- [x] Daftar `/cbt/sessions` menambahkan filter cepat `Semua`, `Belum Siap`, `Siap Mulai`, `Butuh Peserta`, `Butuh Ruang`, dan `Butuh Pengawas`.
- [x] Setiap filter menampilkan jumlah sesi yang cocok, dihitung dari payload readiness yang sudah ada: mutu paket, peserta, kapasitas, penempatan ruang/kursi, dan pengawas.
- [x] Header daftar menampilkan jumlah terlihat dibanding total sesi, dan menyediakan reset filter saat operator sedang fokus pada kategori tertentu.
- [x] Empty state dibedakan antara benar-benar belum ada sesi dan hasil filter yang kosong agar operator tidak salah membaca kondisi data.
- [x] Guardrail: tidak mengubah kontrak backend/Flutter, tidak menambah endpoint atau migrasi, dan tetap seluruhnya melalui data BFF/Go API existing.

### ✅ Sprint 83 — CBT Session Filter URL Persistence (COMPLETE)
- [x] Tujuan utama: membuat filter kesiapan sesi bisa dipertahankan saat refresh, bookmark, atau dibagikan ke operator lain.
- [x] `/cbt/sessions` sekarang membaca query `readiness` saat halaman dibuka dan langsung mengaktifkan filter yang valid.
- [x] Klik filter cepat memperbarui URL dengan `readiness=not_ready|ready|needs_participants|needs_rooms|needs_proctors`; filter `Semua` menghapus query agar URL tetap bersih.
- [x] Nilai query yang tidak dikenal dibersihkan otomatis, sehingga URL lama/typo tidak membuat state UI rancu.
- [x] Guardrail: hanya mengubah state URL client-side, tidak menambah endpoint, tidak menambah storage lokal, dan tidak mengubah kontrak data sesi CBT.

### ✅ Sprint 84 — CBT Session Readiness Board (COMPLETE)
- [x] Tujuan utama: memberi operator papan angka ringkas sebelum tabel agar prioritas sesi langsung terlihat.
- [x] `/cbt/sessions` sekarang menampilkan board compact untuk `Belum Siap`, `Siap Mulai`, `Butuh Peserta`, `Butuh Ruang`, dan `Butuh Pengawas`.
- [x] Setiap kartu board menampilkan jumlah sesi dan keterangan singkat, lalu bisa diklik untuk mengaktifkan filter readiness yang sama.
- [x] State aktif board mengikuti query `readiness`, sehingga refresh/bookmark/share tetap menandai kategori yang sedang dipantau.
- [x] Skeleton loading daftar sesi diselaraskan menjadi lima slot agar transisi awal tidak terasa berubah drastis.
- [x] Guardrail: tidak menambah endpoint, tidak mengubah model data, tidak mengubah kontrak Flutter, dan tetap memakai payload readiness existing dari BFF/Go API.

### ✅ Sprint 85 — CBT Session Schedule Urgency (COMPLETE)
- [x] Tujuan utama: menambahkan dimensi waktu pada papan kesiapan agar operator melihat sesi mana yang mendesak, bukan hanya siap/tidak siap.
- [x] `/cbt/sessions` sekarang punya board `Urgensi Jadwal` untuk `Hari Ini`, `Akan Datang`, dan `Lewat Jadwal`.
- [x] Filter jadwal disimpan di query `schedule=today|upcoming|overdue` dan dapat digabung dengan query `readiness`, misalnya sesi `Belum Siap` yang `Hari Ini`.
- [x] Hitungan board kesiapan mengikuti filter jadwal aktif, dan hitungan board jadwal mengikuti filter kesiapan aktif, sehingga angka tetap relevan dengan konteks operator.
- [x] Tabel desktop dan kartu mobile menampilkan badge waktu pelaksanaan seperti `Hari ini`, `Akan datang`, `Lewat jadwal`, `Dalam rentang`, atau `Riwayat`.
- [x] Guardrail: tidak menambah endpoint, tidak menambah migrasi, tidak mengubah kontrak Flutter, dan semua kalkulasi jadwal dilakukan client-side dari payload sesi existing.

### ✅ Sprint 86 — CBT Overdue Session Quick Actions (COMPLETE)
- [x] Tujuan utama: membuat sesi `Lewat jadwal` bisa segera ditutup dari daftar tanpa operator harus mencari aksi di detail.
- [x] Tabel desktop dan kartu mobile sekarang menampilkan aksi cepat pada badge jadwal untuk sesi lewat jadwal, termasuk `Selesaikan` untuk sesi aktif dan `Buka Detail` sebagai fallback.
- [x] Tombol `Jadwalkan` dan `Mulai` ikut nonaktif ketika sesi sudah `Lewat jadwal`, sehingga operator diarahkan menutup atau memperbaiki sesi alih-alih memulai ujian kedaluwarsa.
- [x] Backend `CbtSession.UpdateStatus(active)` sekarang menolak aktivasi jika `scheduled_end` sudah lewat, sehingga guard tidak hanya bergantung pada frontend.
- [x] Unit test service menutup kasus aktivasi sesi lewat jadwal agar perubahan status tidak menyentuh readiness room maupun write status.
- [x] Guardrail: tidak menambah migrasi atau endpoint baru, tidak mengubah kontrak Flutter, dan tetap memakai mekanisme status existing dengan konfirmasi destructive yang sudah ada.

### ✅ Sprint 87 — CBT Quick Reschedule (COMPLETE)
- [x] Tujuan utama: memberi opsi memperbaiki sesi lewat jadwal tanpa harus membatalkan sesi yang masih sah untuk digeser.
- [x] Query sqlc `UpdateCbtExamSessionSchedule` ditambahkan untuk memperbarui `scheduled_start` dan `scheduled_end` tanpa migrasi baru.
- [x] Backend `UpdateSchedule` hanya menerima sesi `draft` atau `scheduled`, menolak rentang waktu tidak valid, dan menolak jadwal baru yang sudah berakhir.
- [x] Endpoint Go API dan BFF `PATCH /api/cbt/sessions/{id}/schedule` ditambahkan, tetap melalui admin guard dan proxy SvelteKit.
- [x] `/cbt/sessions` sekarang punya panel `Ubah Jadwal Sesi` yang muncul dari aksi cepat `Ubah Jadwal` pada sesi lewat jadwal.
- [x] Unit test service dan handler menutup kontrak update schedule, termasuk guard status sesi.
- [x] Guardrail: tidak menambah storage frontend, tidak mengubah kontrak Flutter, dan tidak mengubah status/peserta/ruang saat jadwal digeser.

### ✅ Sprint 88 — CBT Session Operational Audit Trail (COMPLETE)
- [x] Tujuan utama: membuat perubahan jadwal dan aksi operasional sesi CBT terlihat sebagai jejak audit per sesi, bukan hanya mutasi HTTP global.
- [x] Backend `UpdateSchedule` sekarang mengembalikan jadwal sebelum dan sesudah update agar handler bisa menulis metadata audit `previous_scheduled_*` dan `new_scheduled_*`.
- [x] Handler menulis audit `CBT_SESSION_SCHEDULE_UPDATE` pada entity `cbt_session` setelah jadwal berhasil digeser.
- [x] Endpoint Go API dan BFF `GET /api/cbt/sessions/{id}/audit-logs` ditambahkan untuk membaca audit per sesi melalui `ListEntityAuditLogs`, termasuk audit ruang/handover yang membawa metadata `session_id`.
- [x] Detail sesi `/cbt/sessions/[id]` mendapat tab `Audit Ops` untuk menampilkan waktu, aktor, aksi, dan ringkasan perubahan jadwal/operasional.
- [x] Unit test service dan handler menutup kontrak audit schedule, scope entity audit, dan serialisasi metadata audit.
- [x] Guardrail: tidak menambah tabel baru, tidak mengubah kontrak Flutter, dan audit tetap disimpan di `audit_logs` backend existing.

### ✅ Sprint 89 — CBT Runtime & Operational Priority Hardening (COMPLETE)
- [x] Tujuan utama: menyelesaikan 7 prioritas review CBT dalam satu patch tanpa membongkar ulang modul yang sudah stabil.
- [x] Live exam runtime sekarang menolak jawaban untuk soal yang bukan bagian paket peserta, termasuk jalur Flutter exam API dan jalur operator/session answer API.
- [x] Jawaban, hitungan progres, proctoring status, hasil peserta, dan detail jawaban peserta sekarang disaring melalui paket peserta agar data luar paket tidak ikut terbaca atau dihitung.
- [x] Final submit menjalankan koreksi objektif ulang sebelum menyimpan skor, memakai bobot per soal dari paket, dan menghormati batas waktu sesi/durasi peserta.
- [x] Token ujian diperkuat menjadi random 128-bit, unik, device-bound setelah login, dan tidak lagi ditampilkan ke role non-admin pada list peserta/proctoring/ruang pengawas.
- [x] Event-level results dan exam cards dibatasi ke admin; akses guru ke hasil sesi dicek sebelum metadata sesi dibaca agar tidak bocor lintas mapel/scope.
- [x] Builder paket mendukung bobot per soal dari UI dan backend, dengan validasi bobot 1-100 dan default aman 1 untuk paket lama atau input sederhana.
- [x] Detail sesi menambahkan `Command Center Ujian` yang menggabungkan readiness ruang, pengawas, submit, koneksi, atensi, dan handover menjadi ringkasan operasional satu layar.
- [x] Migration `060_cbt_exam_token_hardening.sql` wajib dijalankan di luar jam ujian aktif; migration ini tidak merotasi token, tetapi menolak token legacy duplikat sehingga repair/regenerasi eksplisit dan cetak ulang kartu hanya diperlukan untuk token terdampak.
- [x] Verification: `make db-sqlc`, `go test ./internal/service ./internal/handler ./internal/middleware`, `go test ./...`, `npm run check`, `flutter test`, dan Svelte autofixer untuk file `.svelte` tersentuh.

#### Rekomendasi Lanjutan Setelah Sprint 89
- [ ] Jalankan migration 060 pada staging terlebih dahulu, repair/regenerasi hanya token terdampak bila preflight menemukan duplikasi, cetak ulang kartu terdampak, lalu lakukan rehearsal login Flutter dengan beberapa perangkat berbeda.
- [ ] Tambahkan smoke test browser untuk alur operator: buat paket berbobot, buka command center, lihat proctoring, dan cek token visibility admin vs guru.
- [ ] Mulai desain extraction `services/cbt-engine` hanya setelah runtime semantics Sprint 89 stabil di rehearsal, agar pemisahan service tidak membawa bug operasional lama.

### ✅ Sprint 90 — CBT Integrity & Secret Redaction Hotfix (COMPLETE)
- [x] Tujuan utama: menutup temuan review kritis pasca Sprint 89 tanpa mengubah alur guru/operator yang sudah berjalan.
- [x] `UpdateParticipantScores` sekarang hanya menghitung ulang `score` dan tidak lagi mengisi `submitted_at`, sehingga klik hitung skor atau grading essay tidak mengunci peserta yang belum submit.
- [x] Duplicate/race final submit sekarang dipetakan ke error domain `ErrExamAlreadySubmit`, sehingga Flutter/operator tetap menerima semantik konflik yang stabil.
- [x] Detail soal hanya menampilkan `answer_key` kepada admin atau guru penulis soal; guru non-penulis tetap bisa melihat metadata soal yang boleh dilihat tanpa membuka kunci jawaban.
- [x] Export CSV bank soal yang membawa kunci jawaban dibatasi admin-only sampai tersedia export guru yang aman/redacted.
- [x] Berita acara/`minutes` untuk guru memakai peserta yang tersaring per guru dan token tetap dikosongkan; admin tetap bisa melihat token untuk kebutuhan cetak operasional.
- [x] File asset soal melalui JWT sekarang wajib admin atau guru penulis soal; akses peserta tetap lewat konteks exam token dan dicek terhadap paket peserta.
- [x] Guardrail test ditambahkan untuk kontrak SQL scoring, redaksi kunci soal, redaksi token minutes, scope asset file, dan duplicate submit.
- [x] Verification: `go test ./internal/handler ./internal/service ./internal/repository/postgres`.

#### Rekomendasi Lanjutan Setelah Sprint 90
- [ ] Tambahkan smoke test browser khusus visibility: admin melihat token/kunci, guru penulis melihat kunci soalnya sendiri, guru lain tidak melihat kunci/token.
- [x] Buat varian export bank soal untuk guru yang redacted atau hanya untuk soal miliknya sendiri agar kebutuhan MGMP/internal tetap aman.
- [ ] Tambahkan rehearsal operasional: grading essay sebelum submit, hitung skor sesi, lalu pastikan peserta yang belum submit masih bisa lanjut ujian.

### ✅ Sprint 91 — CBT Teacher-Safe Question Export (COMPLETE)
- [x] Tujuan utama: membuat tombol export bank soal tetap berguna untuk guru tanpa membuka kunci jawaban milik guru lain.
- [x] Query bank soal sekarang mendukung filter `author_username` eksplisit di `ListCbtQuestionsFiltered` dan `CountCbtQuestionsFiltered`.
- [x] Service `ListCbtQuestionsInput` meneruskan `AuthorUsername` secara typed dan trim-safe ke sqlc.
- [x] Handler `ExportCSV` kembali menerima role CBT, tetapi non-admin wajib punya username dari JWT dan export dipaksa hanya untuk `author_username` miliknya.
- [x] Admin tetap bisa export penuh sesuai filter yang dipilih, termasuk kunci jawaban untuk kebutuhan operator.
- [x] Test handler menutup kontrak bahwa guru export berhasil dengan scope username sendiri dan ditolak jika username tidak tersedia.
- [x] Test service menutup kontrak bahwa filter penulis diteruskan ke list dan count query.
- [x] Verification: `make db-sqlc`, `go test ./internal/handler ./internal/service ./internal/repository/postgres`.

#### Rekomendasi Lanjutan Setelah Sprint 91
- [x] Tambahkan label UI kecil saat guru export: `Export soal saya` agar ekspektasi guru jelas bahwa hasilnya bukan semua bank soal.
- [ ] Tambahkan smoke test browser visibility untuk tombol export admin vs guru setelah data seed rehearsal tersedia.
- [ ] Pertimbangkan export redacted lintas guru hanya untuk reviewer/MGMP jika workflow review membutuhkan bahan diskusi tanpa kunci jawaban.

### ✅ Sprint 92 — CBT Teacher Export UX Label (COMPLETE)
- [x] Tujuan utama: menyelaraskan teks tombol export dengan scope backend Sprint 91 agar guru paham export hanya mengambil soalnya sendiri.
- [x] `/cbt/soal` sekarang mendeteksi role admin dari `data.user.roles`.
- [x] Admin tetap melihat tombol `Export CSV`, sedangkan non-admin CBT melihat `Export Soal Saya`.
- [x] Toast sukses export dibedakan: admin mendapat pesan bank soal umum, guru mendapat pesan `soal saya`.
- [x] Guardrail: tidak mengubah endpoint, tidak mengubah kontrak CSV, dan tidak menambah state persistence frontend.
- [x] Verification: `npx @sveltejs/mcp svelte-autofixer apps/web-admin/src/routes/cbt/soal/+page.svelte`, `npm run check`.

#### Rekomendasi Lanjutan Setelah Sprint 92
- [ ] Jalankan smoke test browser login admin dan guru untuk memastikan label export, hasil CSV, dan visibility kunci jawaban sesuai role.
- [ ] Tambahkan indikator jumlah `soal saya` jika backend nanti menyediakan summary per author tanpa query tambahan berat.

### ✅ Sprint 93 — CBT Export Visibility Test Coverage (COMPLETE)
- [x] Tujuan utama: memberi guardrail otomatis sebelum smoke test browser agar label export dan proxy CSV tidak regress saat role/admin-guru berubah.
- [x] Helper `question-export-ui.ts` ditambahkan untuk menentukan capability export penuh, label tombol, dan pesan sukses dari role aktif.
- [x] `/cbt/soal` memakai helper export yang sama agar label admin/guru tidak tersebar sebagai string manual di halaman besar.
- [x] Unit test helper menutup kasus admin, guru, dan multi-role admin.
- [x] Test BFF proxy menutup kontrak `/api/cbt/questions/export` tetap meneruskan query filter aktif dan memakai stream CSV no-store.
- [x] Guardrail: belum menggantikan smoke test browser rehearsal karena test login real admin/guru tetap butuh data seed runtime.
- [x] Verification: `npx @sveltejs/mcp svelte-autofixer apps/web-admin/src/routes/cbt/soal/+page.svelte`, `npm run test:unit`, `npm run check`.

#### Rekomendasi Lanjutan Setelah Sprint 93
- [ ] Jalankan browser smoke rehearsal dengan akun admin dan guru setelah staging seed siap: label export, isi CSV, detail kunci jawaban, dan token minutes.
- [x] Bila browser smoke stabil, dokumentasikan checklist singkat operator untuk regression manual sebelum ujian besar.

### ✅ Sprint 94 — CBT Smoke Checklist (COMPLETE)
- [x] Tujuan utama: menyediakan checklist manual yang bisa dipakai operator/dev saat staging seed siap, tanpa menunggu pembuatan suite Playwright penuh.
- [x] Dokumen `docs/cbt-smoke-checklist.md` ditambahkan sebagai checklist admin/guru visibility dan runtime integrity.
- [x] Checklist mencakup prasyarat deploy, akun uji admin/guru, label export admin/guru, isi CSV, kunci jawaban detail, token minutes, scoring sebelum submit, dan login token Flutter setelah hardening.
- [x] Checklist juga menentukan evidence wajib: commit, akun role, sesi uji, screenshot, CSV sample, hasil grading sebelum submit, dan keputusan lulus/tunda.
- [x] Guardrail: dokumen ini tidak mengganti test otomatis Sprint 93; checklist dipakai saat data staging/produksi tersedia.
- [x] Verification: `git diff --check`.

#### Rekomendasi Lanjutan Setelah Sprint 94
- [ ] Jalankan checklist `docs/cbt-smoke-checklist.md` di staging dengan data seed nyata dan catat hasilnya di deployment note.
- [x] Kunci bagian stabil yang tidak butuh browser nyata dengan unit test: redirect route lama dan stream CSV template/export.
- [ ] Jika checklist rutin dipakai, pertimbangkan mengubah bagian role login nyata menjadi Playwright smoke test role-based.

### ✅ Sprint 95 — CBT Retired Route Guard Coverage (COMPLETE)
- [x] Tujuan utama: mengurangi risiko regresi Bank Soal setelah `/cbt/questions` dipensiunkan dan `/cbt/soal` dipatenkan sebagai UI tunggal.
- [x] Unit test route lama `/cbt/questions` memastikan selalu redirect 307 ke `/cbt/soal`.
- [x] Redirect mempertahankan `question_id` dan mode yang masih valid (`catalog`, `composer`, `review`, `import`) agar bookmark lama tetap membantu guru.
- [x] Redirect membuang mode eksperimen lama seperti `studio` supaya UI retired tidak hidup kembali secara tidak sengaja.
- [x] Test BFF stream untuk `/api/cbt/questions/template` ditambahkan agar template CSV tetap lewat proxy, bertipe `text/csv`, dan `no-store`.
- [x] Guardrail: test ini melengkapi checklist smoke, tetapi belum menggantikan login admin/guru real di staging.
- [x] Verification: `npm run test:unit`, `npm run check`, `git diff --check`.
...
- [x] Guardrail: perubahan ini bersifat dokumentasi; tidak mengubah kontrak runtime, schema, atau UI.
- [x] Verification: `git diff --check`.

#### Rekomendasi Lanjutan Setelah Sprint 96
- [ ] Jalankan smoke checklist staging dengan akun admin/guru real sebelum ujian besar berikutnya.
- [ ] Saat extraction `services/cbt-engine` benar-benar dimulai, revisi aturan kepemilikan PostgreSQL di `AGENTS.md` sebagai exception sempit, bukan sebelum implementasi.
